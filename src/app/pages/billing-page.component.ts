          ...
          @if (account()) {
            ...
            @if (canSubscribe()) {
              <div class="plans">
                ...
                <button class="plan-option" [class.selected]="selectedPlan() === 'rubrica_team'" (click)="selectedPlan.set('rubrica_team')"><strong>{{ teamPlanLabel() }}</strong><span>{{ planPrice('rubrica_team') }}</span><span>{{ teamPlanHelp() }}</span></button>
              </div>
              @if (selectedPlan() === 'rubrica_team') { <div class="billing-interval"><button type="button" [class.selected]="billingInterval() === 'month'" (click)="billingInterval.set('month')">{{ monthlyLabel() }}</button><button type="button" [class.selected]="billingInterval() === 'year'" (click)="billingInterval.set('year')">{{ annualLabel() }}</button></div> }
            }
            <div class="actions">
...
export class BillingPageComponent implements OnInit {
  ...
  teamHelp(): string { return this.memberLimit() === 10 ? this.local({ 'pt-BR':'O plano Equipe permite até 10 contas ativas, incluindo o administrador.', en:'Team allows up to 10 active accounts, including the administrator.', es:'Equipo permite hasta 10 cuentas activas, incluido el administrador.', 'ja-JP':'チームでは管理者を含め最大10個の有効アカウントを利用できます。' }) : this.local({ 'pt-BR':'O plano Profissional permite até 3 contas ativas, incluindo o administrador.', en:'Professional allows up to 3 active accounts, including the administrator.', es:'Profesional permite hasta 3 cuentas activas, incluido el administrador.', 'ja-JP':'プロフェッショナルでは管理者を含め最大3つの有効アカウントを利用できます。' }); }
  ...
  memberLimit(): number { return this.account()?.current_product_code === 'rubrica_team' ? 10 : 3; }
