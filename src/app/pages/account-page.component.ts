    ...
    <main class="login-layout"><section class="card auth-card">
      ...
      @if (mode === 'register') {
        <form class="form" (ngSubmit)="register()">
          ...
          <fieldset class="form"><legend>{{ i18n.text('optionalIdentity') }}</legend>
      ...
      <p><a routerLink="/login">{{ i18n.text('backToLogin') }}</a></p>
    </section></main>
  `,
})
export class AccountPageComponent implements OnInit {
  ...
  constructor(private readonly route: ActivatedRoute, private readonly api: ApiService, private readonly feedback: FeedbackService, readonly i18n: I18nService) {}
  ...
  register() { return this.action('/auth/register', { name: this.name, email: this.email, password: this.password, preferred_locale: this.i18n.locale(), identity_document_type: this.country ? this.documentType : null, identity_document_country: this.country || null, identity_document_value: this.country ? this.documentValue : null }, this.i18n.text('registrationSent')); }
  ...
  private async action(path: string, body: unknown, message: string) {
    ...
    catch { await this.feedback.error(this.i18n.text('reviewData')); }
  }
}
