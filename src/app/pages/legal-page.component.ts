...
type PageKey = 'contact' | 'privacy' | 'terms' | 'data-deletion';
type Section = readonly [string, string];
interface PageCopy { eyebrow: string; title: string; intro: string; sections: readonly Section[]; }
interface UiCopy { back: string; signIn: string; updated: string; formTitle: string; name: string; email: string; topic: string; sales: string; support: string; privacy: string; message: string; placeholder: string; sent: string; error: string; unavailable: string; sending: string; send: string; }
...
@Component({
  ...
  template: `
    <main class="legal-shell"><nav class="legal-nav"><a class="landing-brand" routerLink="/"><img src="icons/rubrica-mark.png" alt="" /><strong>Rubrica</strong></a><div class="landing-actions"><app-language-picker /><a class="landing-button small" routerLink="/login">{{ ui().signIn }}</a></div></nav>
    ...
    @if (isContact) {
      ...
      <label>{{ ui().topic }}<select name="topic" [(ngModel)]="form.topic" required><option value="sales">{{ ui().sales }}</option><option value="support">{{ ui().support }}</option><option value="privacy">{{ ui().privacy }}</option></select></label><label>{{ ui().message }}<textarea name="message" [(ngModel)]="form.message" required minlength="10" maxlength="3000" rows="6" [placeholder]="ui().placeholder"></textarea></label><label class="contact-honeypot" aria-hidden="true">Website<input name="website" [(ngModel)]="form.website" tabindex="-1" autocomplete="off" /></label><div #turnstileContainer class="contact-turnstile"></div>
...
export class LegalPageComponent implements AfterViewInit {
  ...
  form = { name: '', email: '', topic: 'sales', message: '', website: '' };
  turnstileToken = '';
