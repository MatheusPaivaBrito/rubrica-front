            ...
            <aside class="grid">
              ...
              <article class="card"><h2>Detalhe da solicitação</h2>
                @if (selectedRequest()) {
                  ...
                  @if (requestLink()) { <p class="eyebrow">Link único da solicitação</p><p class="url-box">{{ requestLink() }}</p><p class="muted">Signatários podem assinar; administradores acessam em modo somente leitura.</p><div class="button-row"><button class="button secondary" (click)="copyInvite()">Copiar link</button><button class="button secondary" (click)="openInvite()">Abrir link</button></div> }
                  @if (isAdmin() && hasSignedSigners()) { <hr /><div class="button-row"><button class="button secondary" (click)="previewSignedRequest()">Ver PDF carimbado</button><button class="button secondary" (click)="showEvidence()">Ver evidências</button></div> }
