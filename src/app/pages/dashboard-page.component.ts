    ...
    <main class="shell">
      ...
      @if (userModalOpen()) {
        <div class="modal-backdrop" (click)="closeUserModal()"><section class="app-modal" (click)="$event.stopPropagation()">
          ...
          <form class="modal-body form" (ngSubmit)="createUser()"><div class="form-row"><label>Nome <input name="userName" [(ngModel)]="userName" required /></label><label>E-mail <input name="userEmail" type="email" [(ngModel)]="userEmail" required /></label></div><label>Senha inicial <input name="userPassword" type="password" [(ngModel)]="userPassword" minlength="8" required /></label><fieldset class="form"><legend>Documento de identificação (opcional)</legend><div class="form-row"><label>País emissor <input name="userIdentityCountry" [(ngModel)]="userIdentityCountry" (ngModelChange)="userIdentityCountryChanged()" list="admin-country-options" maxlength="2" placeholder="BR, JP, PT…" /></label><label>Tipo <select name="userIdentityType" [(ngModel)]="userIdentityType" [disabled]="!userIdentityCountry">@for (option of userIdentityOptions(); track option.value) { <option [value]="option.value">{{ option.label }}</option> }</select></label></div><datalist id="admin-country-options"><option value="BR">Brasil</option><option value="JP">Japão</option><option value="PT">Portugal</option><option value="US">Estados Unidos</option></datalist>@if (userIdentityCountry) { <label>Número do documento <input name="userIdentityValue" [(ngModel)]="userIdentityValue" minlength="4" maxlength="80" autocomplete="off" required /><small>O valor completo será criptografado e nunca será exibido novamente.</small></label> }</fieldset><label>Perfil <select name="userRole" [(ngModel)]="userRole"><option value="signature_signer">Signatário</option><option value="signature_operator">Operador</option><option value="signature_auditor">Auditor</option><option value="signature_admin">Administrador</option></select><small class="role-help">{{ roleDescription() }}</small></label><footer class="modal-footer"><button type="button" class="button secondary" (click)="closeUserModal()">Cancelar</button><button class="button" [disabled]="submitting()">Criar usuário</button></footer></form>
        </section></div>
      }
...
export class DashboardPageComponent implements OnInit {
  ...
  userIdentityOptions(): { value: string; label: string }[] { if (this.userIdentityCountry === 'BR') return [{ value: 'BR_CPF', label: 'CPF' }, { value: 'PASSPORT', label: 'Passaporte' }, { value: 'OTHER', label: 'Outro' }]; if (this.userIdentityCountry === 'PT') return [{ value: 'PT_NIF', label: 'NIF' }, { value: 'PASSPORT', label: 'Passaporte' }, { value: 'OTHER', label: 'Outro' }]; if (this.userIdentityCountry === 'JP') return [{ value: 'PASSPORT', label: 'Passaporte' }, { value: 'RESIDENCE_CARD', label: 'Residence Card' }, { value: 'DRIVER_LICENSE', label: 'Carteira de habilitação' }, { value: 'OTHER', label: 'Outro' }]; return [{ value: 'PASSPORT', label: 'Passaporte' }, { value: 'NATIONAL_ID', label: 'Documento nacional' }, { value: 'RESIDENCE_CARD', label: 'Cartão de residência' }, { value: 'DRIVER_LICENSE', label: 'Carteira de habilitação' }, { value: 'TAX_ID', label: 'Identificação fiscal' }, { value: 'OTHER', label: 'Outro' }]; }
