...
export class DashboardPageComponent implements OnInit, OnDestroy {
  ...
  private billingFileLimit(account: BillingAccount): number | null { if (typeof account.files_limit === 'number') return account.files_limit; if (!['active', 'past_due'].includes(account.status)) return null; if (account.current_product_code === 'rubrica_base') return 25; if (account.current_product_code === 'rubrica_intermediate') return 30; return null; }
