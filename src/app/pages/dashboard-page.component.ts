...
export class DashboardPageComponent implements OnInit, OnDestroy {
  ...
  signerName = '';
  signerEmail = '';
  signerLocale: 'pt-BR' | 'en' | 'es' | 'ja-JP' = 'en';
  signerParticipantRole: 'external_signer' | 'company_representative' = 'external_signer';
  title = '';
  expiresAt = '';
  file: File | null = null;
  private tenantSlug = '';
  ...
  participantTypeHelp(): string { return this.signerParticipantRole === 'company_representative' ? this.localized({ 'pt-BR':'A assinatura vinculará o CPF da pessoa ao CNPJ desta empresa.', en:'The signature will bind the person’s CPF to this company’s CNPJ.', es:'La firma vinculará el CPF de la persona al CNPJ de esta empresa.', 'ja-JP':'署名に本人のCPFと会社のCNPJが記録されます。' }) : this.localized({ 'pt-BR':'A pessoa assinará em nome próprio, sem representar esta empresa.', en:'The person will sign in their own name without representing this company.', es:'La persona firmará en nombre propio, sin representar a esta empresa.', 'ja-JP':'会社を代表せず、本人名義で署名します。' }); }
  ...
  private billingFileLimit(account: BillingAccount): number | null { if (typeof account.files_limit === 'number') return account.files_limit; if (!['active', 'past_due'].includes(account.status)) return null; if (account.current_product_code === 'rubrica_base') return 20; if (account.current_product_code === 'rubrica_intermediate') return 80; return null; }
