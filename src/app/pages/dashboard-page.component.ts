...
export class DashboardPageComponent implements OnInit {
  ...
  async ngOnInit(): Promise<void> { const context = await this.auth.restore(); if (!context) { await this.router.navigate(['/login']); return; } try { const dashboardUrl = await this.auth.dashboardUrl(); this.tenantId = dashboardUrl.split('/')[2] ?? ''; if (!this.route.snapshot.paramMap.get('tenantId') && this.tenantId) { await this.router.navigateByUrl(dashboardUrl, { replaceUrl: true }); return; } if (context.mfa_setup_required && !this.auth.isMfaDeferredForSession()) { await this.router.navigate(['/security']); return; } if (this.canManage()) await Promise.all([this.reload(), this.loadBilling()]); } catch (error) { await this.feedback.error(error, this.i18n.text('dashboardLoadFailed')); } finally { this.loading.set(false); } }
  ...
  identityLabel(signer: Signer): string { const type = (signer.identity_document_type ?? '').replace('BR_', '').replace('PT_', '').replaceAll('_', ' '); return `${type} ${signer.identity_document_masked ?? ''}`.trim(); }
