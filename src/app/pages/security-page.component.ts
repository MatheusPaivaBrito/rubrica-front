...
export class SecurityPageComponent implements OnInit {
  ...
  private async navigateAfterSecurity(): Promise<void> { const candidate = this.route.snapshot.queryParamMap.get('returnUrl'); const safeReturn = candidate === '/plan' || candidate === '/dashboard' || Boolean(candidate && (/^\/(?:a|t|tenant)\/[a-zA-Z0-9/_-]+\/dashboard$/.test(candidate) || /^\/signing\/[a-zA-Z0-9_-]+$/.test(candidate))); const destination = safeReturn ? candidate! : await this.auth.dashboardUrl(); await this.router.navigateByUrl(destination, { replaceUrl: true }); }
}
